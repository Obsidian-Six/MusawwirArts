...
const ManageGallery = ({ onEdit }) => {
  ...
  useEffect(() => {
    const onPublished = () => fetchPaintings();
    window.addEventListener('paintingPublished', onPublished);
    return () => window.removeEventListener('paintingPublished', onPublished);
  }, []);
  ...
  const filteredPaintings = Array.isArray(paintings)
    ? paintings.filter(p => {
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700 p-1">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
          <h2 className="text-2xl md:text-3xl font-serif text-stone-900">Live Collection</h2>
          <p className="text-stone-400 text-[10px] md:text-xs uppercase tracking-[0.2em]">
            Archiving {paintings.length} Published Works
          </p>
        </div>
      ...
      <div className="bg-white md:rounded-[2.5rem] rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-20 text-center text-stone-300 italic">
            No published masterpieces match your search.
          </div>
        ) : (
          <>
            {/* DESKTOP VIEW: Table */}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-stone-50">
                    <th className="p-8 text-[10px] uppercase tracking-widest text-stone-400 font-bold">Artwork</th>
                    <th className="p-8 text-[10px] uppercase tracking-widest text-stone-400 font-bold">Details</th>
                ...
                <tbody className="divide-y divide-stone-50">
                  {filteredPaintings.map((painting) => (
                    <tr key={painting._id} className="group hover:bg-stone-50/50 transition-colors">
                      <td className="p-6">
                        <div className="flex items-center gap-6">
                          <div className="w-16 h-16 rounded-xl bg-stone-100 overflow-hidden shadow-inner flex-shrink-0">
                            <img
                              src={getFullImageUrl(painting.imageUrl)}
                              alt={painting.title}
                              className="w-full h-full object-cover grayscale-[0.3] group-hover:grayscale-0 transition-all duration-500"
                              onError={(e) => { e.target.src = 'https://placehold.co/400x400?text=Art+Missing'; }}
                            />
                          </div>
                          <div>
                            <p className="font-serif text-lg text-stone-900 leading-tight">{painting.title}</p>
                            <p className="text-[10px] text-stone-400 uppercase tracking-tighter mt-1">{painting.medium}</p>
                          </div>
                        </div>
                      </td>

                      <td className="p-6">
                        <span className="bg-stone-100 px-3 py-1 rounded-full text-[10px] uppercase font-bold text-stone-500">
                          {painting.category?.name || (typeof painting.category === 'string' ? painting.category : "No Category")}
                        </span>
                        <p className="mt-2 text-[11px] text-stone-400">{painting.dimensions}</p>
                      </td>

                      <td className="p-6">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${painting.isAvailable ? 'bg-emerald-500' : 'bg-stone-300'}`}></div>
                      ...
                      <td className="p-6 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => onEdit(painting)} className="p-3 text-stone-400 hover:text-stone-900 hover:bg-white rounded-xl transition-all shadow-hover">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /><path d="m15 5 4 4" /></svg>
                          </button>
                          <button onClick={() => handleDelete(painting._id)} className="p-3 text-stone-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
            ...
            <div className="md:hidden divide-y divide-stone-100">
              {filteredPaintings.map((painting) => (
                <div key={painting._id} className="p-6 space-y-4">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h3 className="font-serif text-lg text-stone-900 leading-tight">{painting.title}</h3>
                        <div className={`w-2 h-2 rounded-full mt-2 ${painting.isAvailable ? 'bg-emerald-500' : 'bg-stone-300'}`}></div>
                      </div>
                      <p className="text-[10px] text-stone-400 uppercase tracking-widest mt-1">{painting.medium}</p>
                      <div className="mt-2">
                        <span className="bg-stone-100 px-2 py-0.5 rounded-full text-[9px] uppercase font-bold text-stone-500">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <p className="text-[11px] text-stone-400 font-medium">{painting.dimensions}</p>
