...
const SculptureDetailPage = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/sculptures/${id}`);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
        ...
        <div className="lg:col-span-5 space-y-8 font-sans">
          ...
          <div className="grid grid-cols-2 gap-y-8 gap-x-4 pt-8 border-t border-stone-200">
            ...
            <div className="space-y-1">
                <p className="text-[11px] text-stone-400 uppercase tracking-[0.2em] font-bold">Material</p>
                <p className="text-sm text-stone-800 font-medium">{product.material}</p>
            </div>
            {product.weight && (
              <div className="space-y-1">
                  <p className="text-[11px] text-stone-400 uppercase tracking-[0.2em] font-bold">Weight</p>
                  <p className="text-sm text-stone-800 font-medium">{product.weight}</p>
              </div>
            )}
