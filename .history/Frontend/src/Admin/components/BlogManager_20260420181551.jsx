...
import { 
  ...
  Image as ImageIcon, Layers, FileText, ChevronLeft, Tag
} from 'lucide-react';
...
const BlogManager = ({ API_URL }) => {
  ...
  const onSubmit = async (e) => {
    ...
    // 2. Append JSON fields (Matching your controller's JSON.parse logic)
    submitData.append('seo', JSON.stringify(formData.seo));
    
    const categoriesArr = formData.categories.split(',').map(s => s.trim()).filter(Boolean);
    submitData.append('categories', JSON.stringify(categoriesArr));
    ...
    // Filter out files that were newly added vs those that are already on the server
    const newFiles = imageList.filter(img => img.file).map(img => img.file);
    
    if (newFiles.length > 0) {
      // If we have new files, your backend 'updateBlog' deletes old ones and saves these
      newFiles.forEach(file => submitData.append('images', file));
    } else if (editingBlog) {
      // If no NEW files were added, we send the existing paths to prevent the backend 
      // from thinking the gallery is empty (if you adjust backend later to support partial updates)
      const existing = imageList.filter(img => img.remotePath).map(img => img.remotePath);
      submitData.append('existingImages', JSON.stringify(existing));
    }
  ...
  const handleEdit = (blog) => {
    ...
    setFormData({ 
      ...
      tags: (blog.tags || []).join(', '), 
      scheduledAt: blog.scheduledAt?.slice(0, 16) || '',
      seo: blog.seo || initialFormState.seo
    });
    
    // Map existing images so we can track their remote paths
    setImageList((blog.images || []).map(img => ({ 
      src: `${API_URL.replace('/api', '')}${img}`, 
      remotePath: img 
    })));
  ...
  // UI Components (Input, Table, etc.) stay the same as your original code
  const Input = ({ label, icon: Icon, children, ...props }) => {
    const baseClass = "w-full px-4 py-3 bg-stone-50 rounded-xl text-sm focus:ring-2 focus:ring-stone-200 outline-none transition-all";
    return (
      <div className="space-y-1.5">
        ...
        <div className="flex w-full sm:w-auto gap-2">
          {activeTab === 'list' ? (
            <>
              <div className="relative w-full">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-300" size={14} />
                <input placeholder="Search..." className="pl-9 pr-4 py-2 bg-stone-50 rounded-lg text-sm w-full outline-none" onChange={e => setSearchTerm(e.target.value)} />
              </div>
              <button onClick={() => { resetForm(); setActiveTab('form'); }} className="bg-stone-900 text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2">
                <Plus size={16}/> New
              </button>
            </>
          ) : (
            <button onClick={() => { setActiveTab('list'); resetForm(); }} className="text-stone-500 flex items-center gap-2 text-sm">
              <ChevronLeft size={16}/> Back to Library
            </button>
          )}
        </div>
      ...
      {activeTab === 'list' ? (
             ...
             <tbody className="divide-y divide-stone-50">
                {blogs.map(blog => (
                  <tr key={blog._id} className="group hover:bg-stone-50/50">
                    <td className="px-6 py-4 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-stone-100 overflow-hidden border">
                        {blog.images?.[0] && <img src={`${API_URL.replace('/api', '')}${blog.images[0]}`} className="w-full h-full object-cover" />}
                      </div>
                      <div className="font-medium text-sm text-stone-800">{blog.title}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${blog.status === 'published' ? 'bg-green-50 text-green-600' : 'bg-stone-100 text-stone-500'}`}>
                        {blog.status}
        ...
        <form onSubmit={onSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-stone-100 space-y-4">
              <h3 className="text-xs font-bold uppercase text-stone-400 flex items-center gap-2"><Search size={14}/> SEO Optimization</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="Meta Title" value={formData.seo.metaTitle} onChange={e => setFormData({...formData, seo: {...formData.seo, metaTitle: e.target.value}})} />
                <Input label="Custom Slug" value={formData.seo.canonicalUrl} onChange={e => setFormData({...formData, seo: {...formData.seo, canonicalUrl: e.target.value}})} />
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl border border-stone-100 space-y-4">
              <button type="submit" className="w-full py-3 bg-stone-900 text-white rounded-xl font-bold flex items-center justify-center gap-2">
                <Save size={16}/> {editingBlog ? 'Update Post' : 'Publish Story'}
              </button>
              <Input label="Status" type="select" icon={Layers} value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}>
                <option value="draft">Save as Draft</option>
            ...
            <div className="bg-white p-6 rounded-2xl border border-stone-100 space-y-4">
              <label className="text-[10px] font-bold uppercase text-stone-400 flex items-center gap-2"><ImageIcon size={14}/> Gallery (Max 5)</label>
              <div className="grid grid-cols-3 gap-2">
                {imageList.map((img, i) => (
                  <div key={i} className="relative aspect-square rounded-lg bg-stone-50 overflow-hidden border">
                    <img src={img.src} className="w-full h-full object-cover" />
                    <button type="button" onClick={() => setImageList(imageList.filter((_, idx) => idx !== i))} className="absolute top-1 right-1 bg-white/90 rounded-full p-1"><X size={10}/></button>
                  </div>
                ))}
                {imageList.length < 5 && (
                  <label className="aspect-square rounded-lg border-2 border-dashed border-stone-100 flex flex-col items-center justify-center text-stone-300 cursor-pointer hover:bg-stone-50">
                    <Plus size={20}/>
                    <input type="file" multiple className="hidden" onChange={e => {
                      const files = Array.from(e.target.files).slice(0, 5 - imageList.length);
                      setImageList([...imageList, ...files.map(f => ({ src: URL.createObjectURL(f), file: f }))]);
                    }} />
                  </label>
                )}
            ...
            <div className="bg-white p-6 rounded-2xl border border-stone-100 space-y-4">
              <Input label="Categories (CSV)" icon={Tag} value={formData.categories} onChange={e => setFormData({...formData, categories: e.target.value})} />
              <Input label="Tags (CSV)" icon={Tag} value={formData.tags} onChange={e => setFormData({...formData, tags: e.target.value})} />
              <Input label="Author" icon={FileText} value={formData.author} onChange={e => setFormData({...formData, author: e.target.value})} />
            </div>
          </div>
        </form>
      )}
