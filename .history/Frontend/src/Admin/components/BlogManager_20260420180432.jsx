...
const BlogManager = ({ API_URL }) => {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'published': return <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle size={10} /> Published</span>;
      case 'scheduled': return <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold flex items-center gap-1"><Clock size={10} /> Scheduled</span>;
      default: return <span className="px-2 py-1 bg-stone-100 text-stone-500 rounded-full text-xs font-bold flex items-center gap-1"><FileText size={10} /> Draft</span>;
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-stone-100 shadow-sm">
        <div>
          <h2 className="text-2xl font-serif text-stone-900">Content Engine</h2>
          ...
          <div className="flex gap-3">
            <div className="relative group">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2.5 bg-stone-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-stone-200 transition-all w-full md:w-64"
                value={searchTerm}
            ...
            <button 
              ...
              className="px-5 py-2.5 bg-stone-900 text-white rounded-xl text-sm font-medium hover:bg-stone-800 transition-all flex items-center gap-2 shadow-lg shadow-stone-200"
            >
      ...
      {activeTab === 'list' ? (
        <div className="grid grid-cols-1 gap-4">
          {loading ? (
            ...
            <div className="bg-white p-12 rounded-2xl border border-stone-100 text-center space-y-4">
            ...
            {/* Desktop / wide screens: table view */}
            <div className="hidden md:block bg-white rounded-2xl border border-stone-100 overflow-hidden shadow-sm">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-stone-50/50 border-b border-stone-100">
                    <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-stone-400">Article</th>
                    <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-stone-400">Status</th>
                    <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-stone-400">Categories</th>
                    <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-stone-400">Date</th>
                    ...
                    <tr key={blog._id} className="hover:bg-stone-50/50 transition-colors group">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-lg bg-stone-100 flex-shrink-0 overflow-hidden relative border border-stone-200">
                          ...
                          <div>
                            <h4 className="text-sm font-medium text-stone-900 leading-tight">{blog.title}</h4>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-wrap gap-1">
                          {blog.categories.slice(0, 2).map((cat, i) => (
                            <span key={i} className="px-2 py-0.5 bg-stone-100 text-stone-500 rounded text-[10px] font-medium">{cat}</span>
                          ))}
                      ...
                      <td className="px-6 py-5">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(blog)} className="p-2 text-stone-400 hover:text-stone-900 transition-colors"><Edit2 size={16} /></button>
                          <button onClick={() => handleDelete(blog._id)} className="p-2 text-stone-400 hover:text-red-600 transition-colors"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="md:hidden space-y-3">
              {blogs.map((blog) => (
                <div key={blog._id} className="bg-white p-4 rounded-2xl border border-stone-100 shadow-sm flex items-start gap-4">
                  <div className="w-16 h-16 rounded-md bg-stone-100 overflow-hidden flex-shrink-0 border border-stone-200">
                    {(blog.images && blog.images[0]) ? (
                      <img src={`${API_URL.replace('/api', '')}${blog.images[0]}`} alt="" className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-stone-300"><ImageIcon size={18} /></div>
                    )}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="text-sm font-medium text-stone-900">{blog.title}</h4>
                        <p className="text-xs text-stone-400 mt-1 line-clamp-2">{blog.excerpt}</p>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] text-stone-400">{blog.publishedAt ? new Date(blog.publishedAt).toLocaleDateString() : '—'}</div>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleEdit(blog)} className="p-2 text-stone-500 hover:text-stone-900 transition-colors"><Edit2 size={16} /></button>
                        <button onClick={() => handleDelete(blog._id)} className="p-2 text-stone-500 hover:text-red-600 transition-colors"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
          <div className="space-y-8">
            {/* Main Fields */}
            <div className="bg-white p-8 rounded-2xl border border-stone-100 space-y-6 shadow-sm">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-stone-50 border-none rounded-xl text-lg font-serif focus:ring-2 focus:ring-stone-200 transition-all outline-none"
            ...
            {/* SEO Section */}
            <div className="bg-white p-8 rounded-2xl border border-stone-100 space-y-6 shadow-sm overflow-hidden relative">
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl border border-stone-100 space-y-6 shadow-sm shadow-stone-100">
              <button 
                type="submit"
            ...
            {/* Featured Image */}
            <div className="bg-white p-6 rounded-2xl border border-stone-100 space-y-4 shadow-sm shadow-stone-100">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {(showAllImages ? imageList : imageList.slice(0,4)).map((item, i) => (
                        <div key={i} className="relative rounded-md overflow-hidden bg-stone-100">
                          <img src={item.src} alt={`Preview ${i+1}`} className="w-full h-20 object-cover" />
                          <button
                            type="button"
